  ...
  const getWeather = async (location) => {
    ...
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_URL}?${searchType}&appid=${WEATHER_API_KEY}&units=metric&cnt=5&exclude=hourly,minutely`
      );
      console.log(data);
      if (data.cod != 200) {
      ...
      <div className="flex w-3/4 min-h-full rounded-3xl shadow-lg m-auto bg-gray-100">
        ...
        <div className="form-container">
          <div className="flex items-center justify-center">
            <h3
              className="my-auto mr-auto text-xl text-white text-opacity-90 font-bold shadow-md py-1 px-3 
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            >
              ...
              <button type="submit" className="z-10">
                <i
                  className="fas fa-search text-white -ml-10 border-l my-auto z-10 cursor-pointer p-3"
                  aria-hidden="true"
                  type="submit"
                ></i>
              </button>
